...
{
    int nx = dm->nx, ny = dm->ny, nz = dm->nz, ni,nj,idx;
    ...
    for(iter=0; iter<maxiteration; iter++)
    {
        ...
        for(i=1; i<=nx; i++)
        {
            for(j=1; j<=ny; j++){
                ...
                for(k=ista; k<=nz; k+=2){
                    idx = IDX(i,j,k);
                    temp = *COEFF(a_poisson, 1, i, j, k) * sol[idx-ni]
                         + *COEFF(a_poisson, 2, i, j, k) * sol[idx+ni]
                         + *COEFF(a_poisson, 3, i, j, k) * sol[idx-nj]
                         + *COEFF(a_poisson, 4, i, j, k) * sol[idx+nj]
                         + *COEFF(a_poisson, 5, i, j, k) * sol[idx-1]
                         + *COEFF(a_poisson, 6, i, j, k) * sol[idx+1];
                    sol[idx] = omega * (rhs[idx] - temp) / *COEFF(a_poisson, 0, i, j, k) + (1.0-omega)*sol[idx];
                }
            }
        }
        timer_stamp(10,STAMP_COMP);
